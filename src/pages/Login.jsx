...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success && res.data.token && res.data.user && res.data.user.id) {
        localStorage.setItem('token', res.data.token);
        localStorage.setItem('userId', res.data.user.id); // Store userId for dashboard
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 flex items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      <div className="relative w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl">
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-green-200 p-6 sm:p-8 lg:p-10 relative overflow-hidden">
          ...
          <div className="relative text-center mb-6 sm:mb-8">
            <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-green-600 to-green-700 rounded-2xl mb-4 sm:mb-6 shadow-lg">
              <CheckSquare className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
            </div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-800 mb-2">TaskFlow</h1>
            <p className="text-sm sm:text-base text-green-600">Sign in to manage your tasks</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            <div className="relative group">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-green-50 border border-green-200 rounded-xl py-3 sm:py-4 pl-12 pr-4 text-sm sm:text-base text-green-800 placeholder-green-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 group-focus-within:bg-white"
                  value={form.email}
            ...
            <div className="relative group">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-green-50 border border-green-200 rounded-xl py-3 sm:py-4 pl-12 pr-12 text-sm sm:text-base text-green-800 placeholder-green-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300 group-focus-within:bg-white"
                  value={form.password}
                  onChange={handleChange}
                  required
                />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
              <label className="flex items-center">
                <input
                  type="checkbox"
                  className="rounded border-green-300 text-green-600 focus:ring-green-500 focus:ring-2"
                />
